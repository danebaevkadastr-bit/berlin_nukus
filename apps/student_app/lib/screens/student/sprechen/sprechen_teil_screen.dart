...
import 'package:core/services/gemini_live_prompt.dart';
...
class _SprechenTeilScreenState extends State<SprechenTeilScreen> {
              ...
              Navigator.push(
                ...
                MaterialPageRoute(
                  builder: (_) => VoiceAiScreen(
                    ...
                    initialAufgabe: aufgabe,
                    teilTitle: widget.teil.title,
                    teilNumber: widget.teil.teilNumber,
                  ),
                ),
              );
  ...
  String _formatAufgabeInstruction(String teilTitle, SprechenAufgabe aufgabe) {
    ...
    if (aufgabe.partner.isNotEmpty) {
      final userPartner = aufgabe.partner;
      final aiPartner = (userPartner == 'A') ? 'B' : 'A';
      buf.writeln('[ROL MASHQI / TEIL 2 DEUTSCHES ROLLE-SPIEL]');
      buf.writeln('Der Nutzer ist Teilnehmer $userPartner.');
      buf.writeln('DU (AI) SPIELST DIE ROLLE VON TEILNEHMER $aiPartner!');

      SprechenAufgabe? aiAufgabe;
      if (_isMultiTest && _currentTest < widget.teil.tests.length) {
        final testObj = widget.teil.tests[_currentTest];
        for (final a in testObj.aufgaben) {
          if (a.partner != userPartner && a.partner.isNotEmpty) {
            aiAufgabe = a;
            break;
          }
        }
      ...
      if (aiAufgabe != null) {
        buf.writeln('DEIN EIGENER KURZARTIKEL / DEINE ROLLE ALS TEILNEHMER $aiPartner:');
        if (aiAufgabe.meinung != null && aiAufgabe.meinung!.isNotEmpty) {
          buf.writeln('Meinung von Teilnehmer $aiPartner: "${aiAufgabe.meinung}" ${aiAufgabe.author != null ? '(${aiAufgabe.author})' : ''}');
        }
        if (aiAufgabe.author != null && aiAufgabe.author!.isNotEmpty) {
          buf.writeln('Person/Autor $aiPartner: "${aiAufgabe.author}"');
        }
        buf.writeln('Du MUSST diesen Kurzartikel von Teilnehmer $aiPartner in eigenen Worten auf Deutsch zusammenfassen!');
      }

      buf.writeln('TEIL 2 DIALOG-STRUKTUR:');
      buf.writeln('1. Nachdem der Nutzer seinen Artikel (Text $userPartner) zusammenfasst, fasst du DEINEN EIGENEN Kurzartikel (Text $aiPartner) auf Deutsch zusammen.');
      buf.writeln('2. Antworte diplomatisch ("Das stimmt, aber..."), bringe Nachteile und Gegenargumente ein.');
      buf.writeln('3. Diskutiert über die Situation im eigenen Heimatland (im Land des Nutzers) und eure Erfahrungen.');
      buf.writeln('4. Fasse am Ende gemeinsam das Thema zusammen.');
    } else if (teilTitle.toLowerCase().contains('planen')) {
      buf.writeln('[GEMEINSAM ETWAS PLANEN]');
      buf.writeln('Du bist der deutsche Prüfungspartner. Ihr müsst zusammen diese Aktivität planen und alle Stichpunkte durchgehen.');
    }

    if (aufgabe.meinung != null && aufgabe.meinung!.isNotEmpty) {
      buf.writeln('Fikr kartasi (Meinung von Teilnehmer ${aufgabe.partner}): "${aufgabe.meinung}" ${aufgabe.author != null ? '(${aufgabe.author})' : ''}');
    }
    if (aufgabe.author != null && aufgabe.author!.isNotEmpty) {
      buf.writeln('Fikr bildirgan odam haqida ma\'lumot (Ismi, yoshi, kasbi): "${aufgabe.author}"');
    }
    if (aufgabe.keywords.isNotEmpty) {
      buf.writeln('Stichpunkte (Punktlar / Kalit so\'zlar):');
      for (var i = 0; i < aufgabe.keywords.length; i++) {
        buf.writeln('${i + 1}. ${aufgabe.keywords[i]}');
      }
    }
    if (aufgabe.examples.isNotEmpty) {
