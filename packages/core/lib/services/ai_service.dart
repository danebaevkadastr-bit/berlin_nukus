...
class AIService {
  ...
  static String get _proxyUrl {
    final url = (dotenv.env['CF_WORKER_URL'] ?? '').trim().replaceAll(RegExp(r'/+$'), '');
    if (url.isNotEmpty) return url;
    // Fallback for cached or missing env in web
    return 'https://berlinnukus-proxy.danebaevkadastr.workers.dev';
  }

  static String get _appToken {
    final token = (dotenv.env['APP_TOKEN'] ?? '').trim();
    if (token.isNotEmpty) return token;
    // Fallback for cached or missing env in web
    return 'Wor7pFuZSsendpyxDPgx4pHUj7An93sy8gRzO7IVkk4x';
  }
  ...
  /// Gemini birinchi o'ringa qo'yildi — qoraqalpoq (kaa), o'zbek va nemis tillarida eng aniq natija beradi.
...
- Savollar faqat $difficulty darajasiga mos bo'lsin — oson ham, qiyin ham qilmang.''',
  ...
  /// Nemischa so'zni tahlil qilib, artikli, tarjimasi va misol gapini qaytaradi.
  static Future<Map<String, dynamic>> analyzeWord(String word, [String targetLang = 'uz']) async {
    final systemPrompt = '''Sen nemis tili lug'atshunosisan. Berilgan nemis so'zini tahlil qilib JSON formatida javob ber.
Format:
{
  "original": "$word",
  "translation": "tarjima",
  "grammar": "artikl va grammatika",
  "meanings": [
    {
      "translation": "tarjima",
      "grammar": "artikl / ko'plik",
      "exampleGerman": "nemischa misol gap",
      "exampleUzbek": "misol gap tarjimasi"
    }
  ]
}''';
  ...
  /// Kiritilgan matn ichidagi barcha nemischa so'zlarni (yakka so'z yoki 150 tagacha so'zlar ro'yxatini)
  /// AI orqali birma-bir ajratib, har birining artikli, tarjimasi va grammatikasini JSON ko'rinishida qaytaradi.
  static Future<List<Map<String, dynamic>>> analyzeBulkWords(String input, [String targetLang = 'uz']) async {
    const systemPrompt = '''Sen nemis tili bo'yicha professional sun'iy intellekt lug'atshunosisan.
Foydalanuvchi kiritgan matn ichidan BARCHA nemischa so'z, birikma va iboralarni ajratib ol.

MUHIM QOIDALAR:
1. Ajratgichlar (Separators): So'zlar nuqta-vergul (;), vergul (,), yangi qator (\n) yoki tire (-) bilan ajratilgan bo'lishi mumkin. Masalan: "genießen; die Abschlussfahrt; Kroatien; steigen in + Akk.; zwar ... aber;". Har birini alohida so'z/ibora sifatida ajrat!
2. Grammatik birikmalar: "steigen in + Akk.", "weit weg von + Dat.", "zwar ... aber", "entweder ... oder" kabi gramatik ko'rsatkichli iboralarni parchalamasdan yaxlit "original" sifatida saqla.
3. Otlar: Agar ot bo'lsa, artikli bilan ("die Abschlussfahrt", "der Reisebericht", "das Jugendhotel") saqla va grammar maydonida ko'plik shaklini yoz.
4. TARJIMA (JUDA MUHIM): Har bir nemischa so'z uchun "translation" maydonida hamda "meanings" ichidagi "translation" maydonida ALBATTA o'zbekcha aniq tarjimasini yoz. Tarjimani HECH QACHON BO'SH QOLDIRMA!
5. Agar matn ichida o'zbekcha tarjimalar yoki misol gaplar bo'lsa, ularni ham tegishli nemischa so'zga biriktir.

FAQAT to'g'ri JSON formatida javob ber:
{
  "words": [
    {
      "original": "die Abschlussfahrt",
      "translation": "bitiruv sayohati",
      "grammar": "die, -en",
      "meanings": [
        {
          "translation": "bitiruv sayohati",
          "grammar": "die, -en",
          "exampleGerman": "Wir machen eine Abschlussfahrt nach Berlin.",
          "exampleUzbek": "Biz Berlinga bitiruv sayohatiga bormoqdamiz."
        }
      ]
    },
    {
      "original": "steigen in (+ Akk.)",
      "translation": "minmoq, o'tirmoq",
      "grammar": "fe'l boshqaruvi",
      "meanings": [
        {
          "translation": "minmoq, o'tirmoq",
          "grammar": "in + Akk.",
          "exampleGerman": "Er steigt in den Bus ein.",
          "exampleUzbek": "U avtobusga minmoqda."
        }
      ]
    }
  ]
    ...
    try {
      final decoded = jsonDecode(_extractJson(raw));
      if (decoded is Map && decoded.containsKey('words') && decoded['words'] is List) {
        final list = decoded['words'] as List<dynamic>;
        return list.map((e) => Map<String, dynamic>.from(e as Map)).toList();
      }
    } catch (e) {
      debugPrint('analyzeBulkWords parse error: $e');
    }
...
Min: $minWords, Max: $maxWords''',
  ...
  /// Gemini Live suhbati transkripsiyasini TELC B1/B2 mezonlari bo'yicha baholash.
  static Future<SprechenLiveEvaluation> evaluateLiveSprechenSession({
    ...
    try {
      final formattedHistory = history.map((h) => '${h['speaker']}: "${h['text']}"').join('\n');
      final langName = (uiLangCode == 'uz')
          ? 'Usbekisch (o\'zbek tilida)'
          : (uiLangCode == 'kaa')
              ? 'Karakalpakisch (qaraqalpaqsha)'
              : (uiLangCode == 'ru')
                  ? 'Russisch'
                  : 'Deutsch';

      final maxPoints = (level.toUpperCase().contains('B2'))
          ? 25
          : (teilNumber == 1 ? 15 : 30);
        ...
        messages: [
          {
            'role': 'system',
            'content': '''
Du bist ein offizieller Prüfer für die mündliche TELC/Goethe-Deutschprüfung ($level).
Der Lernende hat gerade Teil $teilNumber ($teilTitle - $taskTitle) absolviert.

Hier ist das GESAMTE TRANSKRIPT des echten Gesprächs:
$formattedHistory

Aufgabe/Thema: $taskTitle
Maximal erreichbare Punkte für diesen Teil: $maxPoints Punkte.

Deine Aufgabe:
Analysiere das GESAMTE Gespräch streng nach den offiziellen TELC $level Bewertungskriterien für Sprechen:
1. Aufgabenbewältigung (Thema & Stichpunkte abgedeckt)
2. Ausdrucksfähigkeit (Wortschatz & passende Redemittel)
3. Formale Richtigkeit (Grammatik & Satzbau)
4. Aussprache & Flüssigkeit

EXTREM WICHTIG: Der "score" Wert DARF NIEMALS größer als $maxPoints sein! Maximal erreichbar: $maxPoints Punkte.

ANTWORTE AUSSCHLIESSLICH IM FOLGENDEN JSON-FORMAT (kein anderer Text):
{
  "score": ${(maxPoints * 0.7).round()},
  "maxScore": $maxPoints,
  "gradeLabel": "TELC $level - Gut Bestanden",
  "status": "Bestanden",
  "criteriaScores": {
    "Aufgabenbewältigung": "${(maxPoints * 0.3).round()} / ${(maxPoints * 0.35).round()}",
    "Ausdrucksfähigkeit": "${(maxPoints * 0.2).round()} / ${(maxPoints * 0.3).round()}",
    "Formale Richtigkeit": "${(maxPoints * 0.1).round()} / ${(maxPoints * 0.2).round()}",
    "Flüssigkeit": "${(maxPoints * 0.1).round()} / ${(maxPoints * 0.15).round()}"
  },
  "overallFeedback": "Suhbat bo'yicha umumiy xulosa va tavsiyalar...",
  "strengths": [
    "Rejadagi barcha punktlarni qamrab oldingiz",
    "Bog'lovchilardan (weil, obwohl) foydalandingiz"
  ],
  "corrections": [
    {
      "originalText": "Ich habe nach Berlin gegangen",
      "correctedText": "Ich bin nach Berlin gegangen",
      "explanation": "Harakat fe'llari 'sein' bilan keladi."
    }
  ],
  "suggestedRedemittel": [
...
WICHTIG:
- Der "score" DARF NIEMALS $maxPoints überschreiten! Wenn maxScore $maxPoints ist, darf score maximal $maxPoints sein.
- Die Summe aller criteriaScores soll ungefähr dem score entsprechen.
- Erkläre die Fehler und Feedback in der Sprache des Nutzers ($langName).
- Finde ALLE ECHTEN Grammatik-, Wortschatz- und Satzbaufehler des Nutzers. Erfinde keine Fehler!''',
          },
          {
            'role': 'user',
            'content': 'Bitte bewerte die Leistung des Nutzers aus diesem Transkript.',
          },
        ],
      );
      ...
      final decoded = jsonDecode(_extractJson(raw));
      if (decoded is! Map) throw Exception('Invalid JSON root');
      return SprechenLiveEvaluation.fromJson(Map<String, dynamic>.from(decoded));
    } catch (e) {
      debugPrint('evaluateLiveSprechenSession error: $e');
      rethrow;
    }
  }
