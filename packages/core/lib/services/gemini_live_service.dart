...
import 'sprechen_live_prompt.dart';
...
class GeminiLiveService {
  ...
  static const Duration _micFlushInterval = Duration(milliseconds: 250);
  ...
  /// Joriy sessiyadagi barcha suhbat matni (user va AI turn'lari).
  final List<Map<String, String>> sessionTranscript = [];
  ...
  Future<void> connect({
    ...
    try {
      ...
      try {
        ...
        final isSprechen = _dynamicTaskInstruction != null && _dynamicTaskInstruction!.trim().isNotEmpty;
          ...
          'setup': {
            ...
            'realtimeInputConfig': {
              'automaticActivityDetection': {
                ...
                'silenceDurationMs': isSprechen ? 12000 : 3500,
              },
            },
            'systemInstruction': {
              'parts': [
                {
                  'text': (_dynamicTaskInstruction != null && _dynamicTaskInstruction!.trim().isNotEmpty)
                      ? buildSprechenLivePrompt(
                          uiLangCode: uiLangCode,
                          dynamicTaskInstruction: _dynamicTaskInstruction!,
                          userName: _userName,
                        )
                      : buildGeminiLivePrompt(
                          uiLangCode: uiLangCode,
                          mode: mode,
                          customPersonality: customPersonality,
                          userName: _userName,
                          dynamicGlossary: glossary,
                          dynamicTaskInstruction: null,
                        )
                },
              ],
            },
    ...
    try {
      ...
      if (serverContent != null) {
        ...
        if (inT != null && inT['text'] != null) {
          final uText = (inT['text'] as String).trim();
          if (uText.isNotEmpty) {
            debugPrint('GeminiLive: USER dedi => "$uText"');
            // Foydalanuvchi gapira boshlasa va AI o'sha paytda gapirayotgan bo'lsa, AI'ni darhol to'xtatamiz
            if (state.value == AiFaceState.speaking) {
              interrupt();
            }
            sessionTranscript.add({'speaker': 'User', 'text': uText});
            _resetSilenceTimer();
          }
        }
        ...
        if (serverContent['turnComplete'] == true) {
          final fullText = _turnText.toString().trim();
          if (fullText.isNotEmpty) {
            sessionTranscript.add({'speaker': 'AI', 'text': fullText});
          }
  ...
  void _sendOpeningTrigger() {
    ...
    if (_currentMode == VoiceAiMode.lugat) {
      trigger = 'Hallo! Ich bin bereit für mein Vokabel-Quiz. Bitte frage mich SOFORT auf Deutsch nach der usbekischen Übersetzung des ersten Wortes aus meiner Vokabelliste!';
    } else if (_dynamicTaskInstruction != null && _dynamicTaskInstruction!.trim().isNotEmpty) {
  ...
  /// Sessiya davomida botga matnli ko'rsatma yuborish (masalan vaqt tugashiga 10s qolganda yakunlash ko'rsatmasi).
  void sendTextPrompt(String text) {
    final ch = _channel;
    if (ch != null && _connected) {
      try {
        ...
        debugPrint('GeminiLive: matnli ko\'rsatma yuborildi: $text');
      } catch (e) {
        debugPrint('GeminiLive: sendTextPrompt xatosi: $e');
      }
    }
  }
  ...
  void _resetSilenceTimer() {
    ...
    if (!_connected || _closed || _dynamicTaskInstruction == null) return;

    _silenceTimer = Timer(const Duration(seconds: 9), () {
      if (_connected && !_closed && state.value == AiFaceState.listening) {
        debugPrint('GeminiLive: Jimlik aniqlandi (9s) -> yordamchi prompt yuborilmoqda');
        sendTextPrompt(
          'SYSTEM-HINWEIS: Der Nutzer schweigt/zögert seit 8 Sekunden. Übernimm als Prüfungspartner freundlich die Initiative und stelle ihm eine kurze, gezielte Nachfrage auf Deutsch zum Thema!',
        );
      }
    });
  }
  ...
  void _flushMicBuffer() {
    ...
    // Agar AI gapirayotgan bo'lsa, mikrofon audio paketlarini yubormaymiz.
    // Bu echoCancel o'chirilganda AIning o'z ovozidan o'zini to'xtatib qo'yishini oldini oladi.
    if (state.value == AiFaceState.speaking) {
      return;
    }
