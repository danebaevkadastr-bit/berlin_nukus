...
class _VideoPlayerScreenState extends State<VideoPlayerScreen> {
  ...
  void _initYoutubePlayer() {
    ...
    // Sinxron subtitr timerini ulash
    _syncTimer = Timer.periodic(const Duration(seconds: 1), (t) async {
      if (!mounted) return;
      try {
        final double curTime = await _ytController.currentTime;
        final int sec = curTime.toInt();
        ...
        setState(() {
          ...
          for (int i = 0; i < widget.video.subtitles.length; i++) {
            ...
            if (sec >= seg.startTimeSec && sec <= seg.endTimeSec) {
